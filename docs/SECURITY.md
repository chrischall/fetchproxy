...
**It does not close the attack, and the difference matters.** The signature covers the two nonces and NOT `ready.extensionSessionPub`. So a concentrator that relays the genuine hellos and the genuine signature — leaving every nonce and identity intact — can still swap the ephemeral public key in the `ready` frame for one it holds the private half of, derive the same shared secret as the peer, and read and rewrite the traffic. Everything the peer checks still passes, because none of it commits to the key actually used for the ECDH. The same is true of the host's own session, and has been since 0.4.0.

What 1.12.0 buys, precisely: an attacker must be positioned to relay a live session between the real extension and the MCP. It can no longer simply *be* the extension, which is what an unauthenticated `ready` allowed. Closing the rest requires the signature to cover the ephemeral key, which is a wire change on both sides — tracked as its own issue rather than smuggled in here.
