...
**Defense — identity-keyed SAS pair prompt.** Every MCP holds a long-term Ed25519 + X25519 keypair at `~/.fetchproxy/identity/<server-name>.json` (mode `0600`). The hello frame carries the public keys, a fresh `sessionNonce`, this session's X25519 ephemeral public key, and an Ed25519 signature over `mcpId || sessionNonce || sessionPub || answersExtNonce` (`helloSignaturePayload` in `@fetchproxy/protocol`; up to protocol 3 the signature covered `mcpId || sessionNonce` and there was no ephemeral to cover). The extension verifies the signature, then looks up the identity hash in `chrome.storage.local["trustedMcps"]` — and, since 3.0.0, requires the stored `identityEd25519Pub` to be the key that just signed, which under protocol 3 it did not have to. [§T8](#t8--mcp-impersonation) says why that half was harmless then and is impersonation now.
