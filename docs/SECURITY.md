...
- nothing a content script writes under `trustedMcps`, `remoteBridges`, `dismissedScopeHashes` or `extensionIdentity` in `storage.local` is read, except by the one-time migration below, which only an upgrade from 3.2.0 or earlier can trigger — a planted record is not trusted, a planted bridge is not dialled, a planted dismissal suppresses nothing, and losing the vault does not re-open the import (`tests/vault-migration.test.ts`);
