...
| `port` | `number` | `37149` (or `FETCHPROXY_WS_PORT`) | Localhost concentrator port; falls back to `FETCHPROXY_WS_PORT` when the option is omitted, which is the only route for a `@fetchproxy/bootstrap` consumer. On one machine every MCP shares the concentrator, so move it for ALL of them or not at all — a port per MCP is meaningful only in a hosted deployment, where a relay agent dials each child and the browser reaches them through a remote target. |
| `host` | `string` | `'127.0.0.1'` (or `FETCHPROXY_WS_HOST`) | Bind interface. Keep on loopback — the threat model assumes single-user trust. Falls back to `FETCHPROXY_WS_HOST` (a literal IP only; anything else is ignored) when the option is omitted, which exists for ONE topology: a hosted child in its own network namespace, where the relay agent dials it on a private veth address (see SECURITY.md). |
| `identityDir` | `string` | `~/.fetchproxy/identity/` (or `FETCHPROXY_IDENTITY_DIR`) | Where to store the long-term identity keypair. Falls back to `FETCHPROXY_IDENTITY_DIR` (an absolute path only; anything else is ignored) when the option is omitted, which is the only route for a `@fetchproxy/bootstrap` consumer. That exists for ONE topology: a host that runs a separate child per CALLER, where each child gets its own `$HOME` and therefore its own identity — so the extension, whose trust store is keyed by the sha256 of that identity, asks to pair with every one of them. The identity answers *which MCP* the browser is talking to, so a host that knows the callers are one MCP points them all at one directory. ([#316](https://github.com/chrischall/fetchproxy/issues/316)) |
